...
The basic structure of our system can be seen in Figure~\ref{fig:systemStructure}. The basic components are the AST, CFG, SSA, Preprocessing and Type Inference. Each component will be described here. \\
...
The preprocessing step is designed to modify the AST generated
...
Join points \\
ssa-ing builtin function names

\subsubsection{Global and Local Variables}
We consider a global variable to be a variable in the Module-wide or Class-wide namespace. A local variable is one which is limited to the scope of a function. \\
...
The keyword \textit{global} in Python is used to enable the programmer to write to the global namespace within a function. By default the programmer has immediate read access to global and local scope and so the keyword is only necessary when a write to the global scope is to take place. With this keyword global variables and global functions can be defined dynamically at runtime. \\
Python code in which the global keyword is used \textit{after} the variable has been assigned, such as the following:
\begin{lstlisting}
    def f(self):
      x = 5
      global x
\end{lstlisting}
functions correctly, i.e.\ $x$ in the global namespace is assigned $5$. However the Python interpreter issues a warning to the programmer. For this reason we do not support this behaviour.
...
We simulate this flow of types with a simple subset constraint. The set of types the RHS can assume must be a subset of the set of types the LHS can assume.
