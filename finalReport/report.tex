...
\begin{titlepage}
 ...
 \noindent\begin{minipage}[t]{\textwidth}
  ...
  Imperial College London
   \par Department of Computing
 \end{minipage}
 \null \vskip 9em
 \begin{center}
  {\huge Practical Types for Python\par}
  \vskip 3em
  ...
  \end{tabular}\par}
  \vskip 1.5em
  {\Large September 2014 \par}
  \vskip 1.5em
  {\Large Supervised by Dr. Jeroen Ketema and  Dr. Alastair Donaldson \par}
  \vskip 10em
 \end{center}\par
 \noindent\begin{minipage}[b]{\textwidth}
  \centering
  Submitted in partial fulfilment of the requirements for the MSc Degree in Computing (Software Engineering) of  Imperial College London
...
\indent Tools attempting to provide the benefits of a statically typed language to Python source code do exist, such as the popular Pylint~\cite{pylint} and PyChecker~\cite{pychecker}, however their type checking ability is limited. More thorough type checkers have been designed, such as Pyty~\cite{pyty} and Pyntch~\cite{pyntch}, however their usability is hampered by the the amount of configuration needed and the number of false positives returned, respectively. \\
...
\begin{lstlisting}[mathescape]
    ...
    x = 'a'
\end{lstlisting}
The variable \texttt{x} will always have the type \texttt{\{int, string\}} after analysing the second statement. \\
...
\indent The basic structure of our system can be seen in Figure~\ref{fig:systemStructure}. The basic components are the Preprocessing, CFG generation, SSA renaming and Type Inference. Each component will be described here. \\
\indent This traverser used to navigate the AST is taken from Edward K. Ream's \textit{stc}~\footnote{\url{https://code.launchpad.net/~edreamleo/python-static-type-checking/trunk} Revision \#448 was used for this project.}.


\begin{figure}
\centering
\includegraphics[scale=0.55]{images/systemStructure.pdf}
...
\begin{lstlisting}[mathescape]
    ...
    try:
      ...
      file.close()
\end{lstlisting}
The code ensures the object is constructed before use, due to flow restrictions, and destroyed after being used since all paths exiting the \texttt{try} must lead to the \texttt{finally}. Python 2.5 introduced the \texttt{with} keyword to make this code cleaner. Using \texttt{with}, the example can be rewritten:
\begin{lstlisting}[mathescape]
...
\indent Python allows an \texttt{else} block to be added at the end the loop. An example of this is as follows:
\begin{lstlisting}
...
\end{lstlisting}
The \texttt{else} body is always entered if the loop test evaluates to false. In the case of \texttt{for} loops reaching the end of the specified iteration is equivalent to false. The \texttt{else} block is not entered when a \texttt{break}, \texttt{exception} or a \texttt{return} occurs. \\
...
\indent The keywords \texttt{finally} and \texttt{except} can be simultaneously used with a \texttt{try}. In this case the relevant \texttt{except} block is executed first and then passes control to the \texttt{finally} block. \\
...
Python provides highly dynamic built-in functions. We will describe the most common of these, how we accommodate them and any problems they may cause. 
...
\indent The larger projects have largely been taken from the beginning of the list of all open source Python3 projects on the Python package index, Pypi~\cite{python3Projects}.
