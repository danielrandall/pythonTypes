...
\indent All but \texttt{int}, \texttt{float} and \texttt{bytes} have builtin functions which can be called. These are supported, as far has been deemed productive and necessary, and the complete list can be seen in (appendix - to do). On top of these types we make use of a special type, \texttt{any} which represents the fact the variable could be any possible type and as such is permitted in all circumstances. \\
...
Python has quite an extensive library available. These include modules, classes and functions.
...
\begin{itemize}
	\item Whether an identifier is used before initialisation
