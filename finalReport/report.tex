...
\indent The larger projects have been taken from the beginning of the list of all open source Python3 projects on the Python package index, Pypi~\cite{python3Projects}.
    ...
    \hline
    \textbf{Name} & \textbf{Lines of Code} & \textbf{Number of Classes} & \textbf{Number of Functions} & \textbf{Runetime} & \textbf{Number of False Positives} \\ \hline
    \end{tabular}
    \caption{The results of the tool being run on a number of different source of Python code.}
	\label{table:toolPerformance}
    \end{table}
...
Some variation of a container type has been used in every sample of Python source code we have taken. \\
\indent Containers are frequently iterated over in \texttt{for} loops. This done in the following way: \texttt{for item\_name in container\_name}. The \texttt{item} is then used in the body of the loop. Since we are unable to infer the types contained in the object we are required to assign \texttt{any} to the item. The use of the item in the body then causes the \texttt{any}s to propagate. Consider the following snippet of code: ...
...
\end{itemize}
A this time our tool is not accurate enough to currently be used as a full-blown bug checker. However it is capable of finding basic programming errors. These include mis-typing variable names, using the wrong variable name in a statement.
